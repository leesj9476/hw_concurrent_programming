				...
				request is really for a 'gap' type lock */
{
	...
	if (trx != lock2->trx && 
		!lock_mode_compatible(static_cast<lock_mode>(LOCK_MODE_MASK & type_mode), lock_get_mode(lock2))) {
	...
	ulint		size)
{
	...
	// rec_cached -> next free rec lock in pool
	// rec_pool   -> pre_allocated record locks
...
RecLock::add_to_waitq(const lock_t* wait_for, const lock_prdt_t* prdt)
{
	...
	// trx_is_high_priority return false in any case
	// so high_priority always has false val
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	// trx_t has their own mutex each
	// return transaction struct implemented by thread
	trx_t*	trx = thr_get_trx(thr);

	lock_rec_req_status	status = LOCK_REC_SUCCESS;

	// we need to create new lock
	if (lock == NULL) {
		// the case if explicit lock
		if (!impl) {
			// create record lock
			RecLock	rec_lock(index, block, heap_no, mode);

			/* Note that we don't own the trx mutex. */
			// use trx_mutex to add lock
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	if (lock == NULL) {
		// the case if explicit lock
		// we called this function by assignment impl(FALSE)
		if (!impl) {
			// create record lock
			RecLock	rec_lock(index, block, heap_no, mode);
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	common cases */
	// we need to change these functions to meet atomic lock insert
	/////////////////////////////////////////////////////////////////
...
/*==========*/
	bool			impl,	/*!< in: if true, no lock is set
					...
					assume that the caller will
					set an implicit lock*/ 
					// the caller set it to FALSE -> use lock
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	/* We try a simplified and faster subroutine for the most
	common cases */
	// use atomic insert(CAS)
	switch (my_lock_rec_lock_fast(impl, mode, block, heap_no, index, thr)) {
	case LOCK_REC_SUCCESS:
		return(DB_SUCCESS);
	case LOCK_REC_SUCCESS_CREATED:
		return(DB_SUCCESS_LOCKED_REC);
...
@return DB_SUCCESS, DB_SUCCESS_LOCKED_REC, DB_LOCK_WAIT, DB_DEADLOCK,
or DB_QUE_THR_SUSPENDED */
// only difference is absence of using of global lock
dberr_t
my_lock_clust_rec_read_check_and_lock(
/*===============================*/
	ulint			flags,	/*!< in: if BTR_NO_LOCKING_FLAG
					bit is set, does nothing */
	const buf_block_t*	block,	/*!< in: buffer block of rec */
	...
	que_thr_t*		thr)	/*!< in: query thread */
{
	...
	// get record lock atomically with no global lock use
	////////////////////////////////////////////////////////////////////////////
	err = my_lock_rec_lock(FALSE, mode | gap_mode, block, heap_no, index, thr);
	////////////////////////////////////////////////////////////////////////////
